...
The real remote-agent path, the real local-agent path, and the live hybrid
local route are verified on the Radxa 5T. The hybrid API route is also
verified up to the remote provider boundary; the current blocker is the remote
provider's region policy, not hybrid-router control flow. The validated local
workflow uses the repo-local Ollama runtime plus the small-model alias
`qwen2.5:0.5b-nb`.
