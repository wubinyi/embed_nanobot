...
- **`ble.py`**: `BLEBridge` — passive BLE advertisement scanning for battery-powered sensors; `BLEScanner` ABC with `BleakBLEScanner` (optional `bleak` dep) and `StubScanner` (testing); `BLEDeviceProfile` with regex name matching and byte-level decode rules (7 data types); periodic scan loop, auto-registration in device registry, state update callback for pipeline + automation; configurable scan interval/duration/device timeout via JSON config
